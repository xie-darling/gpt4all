...
bool starcoder_model_load(const std::string & fname, starcoder_model & model, gpt_vocab & vocab) {
    ...
    // prepare memory for the weights
    {
        ...
        const int head_dim = n_embd / hparams.n_head;
        const int kv_heads = 1; // 1 for MQA
        const int kv_dim   = kv_heads * head_dim;
    ...
    // load weights
    {
        ...
        while (true) {
            ...
            // Special case for MQA
            if (!name.ends_with("/attn/c_attn/w") && !name.ends_with("/attn/c_attn/b")) {
            ...
            // We have to manage the attn tensors by reading manually from the offset
            const auto & hparams = model.hparams;
            if (name.ends_with("/attn/c_attn/w")) {
                // WARNING: This will not work for k quants.
                const size_t blk_per_tok = hparams.n_embd * hparams.n_embd / 32; /*weights per block*/
                const size_t byte_per_tok = blk_per_tok * ggml_blck_size(tensor->type);
                const size_t offset = byte_per_tok;
                const size_t head_dim = hparams.n_embd * hparams.n_embd / hparams.n_head;
                const size_t blk_per_head = head_dim / 32; /*weights per block*/
                const size_t bytes_per_head = blk_per_head * ggml_blck_size(tensor->type);
                const size_t read_size = bytes_per_head;
                fin.read(reinterpret_cast<char *>(tensor->data) + offset, read_size);
                fin.read(reinterpret_cast<char *>(tensor->data) + offset * 2, read_size);
            } else if (name.ends_with("/attn/c_attn/b")) {
                // WARNING: This will not work for k quants.
                const size_t blk_per_tok = hparams.n_embd / 32; /*weights per block*/
                const size_t byte_per_tok = blk_per_tok * ggml_blck_size(tensor->type);
                const size_t offset = byte_per_tok;
                const size_t head_dim = hparams.n_embd / hparams.n_head;
                const size_t blk_per_head = head_dim / 32; /*weights per block*/
                const size_t bytes_per_head = blk_per_head * ggml_blck_size(tensor->type);
                const size_t read_size = bytes_per_head;
                fin.read(reinterpret_cast<char *>(tensor->data) + offset, read_size);
